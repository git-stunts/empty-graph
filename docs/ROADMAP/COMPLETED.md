...
| B87  | ~~**CODE SAMPLE LINTER**~~ — **DONE (`feature/b87-markdown-code-lint`, pending merge).** Added `scripts/lint-markdown-code-samples.js` and `npm run lint:md:code`, which extract fenced JavaScript/TypeScript samples from Markdown and syntax-check them with line-accurate diagnostics. Wired into the CI fast gate and local pre-push firewall alongside markdownlint, with failures for malformed mixed-marker fences and unterminated JS/TS blocks. |
