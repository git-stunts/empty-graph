...
| B166 | **ADAPTER CLEANUP CONTRACTS** — `NodeWsAdapter.close()` doesn't reset `state.wss`/`state.httpServer`/remove listeners after shutdown; `listenWithHttp` error path leaks partial state; `WarpSocket` (demo) `onDiff`/`onDisconnect`/`onError` add listeners with no removal path; failed `connect()` doesn't clean up. **Files:** `src/infrastructure/adapters/NodeWsAdapter.js`, `demo/browsa/src/net/WarpSocket.js`, `demo/browsa/src/stores/graphStore.js` | M |
