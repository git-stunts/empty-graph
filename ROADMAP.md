...
| B58 | **DEAD SYNC CONSTANTS IN `_internal.js`** — `DEFAULT_SYNC_SERVER_MAX_BYTES`, `DEFAULT_SYNC_WITH_RETRIES`, `DEFAULT_SYNC_WITH_BASE_DELAY_MS`, `DEFAULT_SYNC_WITH_MAX_DELAY_MS`, `DEFAULT_SYNC_WITH_TIMEOUT_MS` plus re-exports of `SyncError` and `OperationAbortedError` are orphaned after SyncController extraction; no importer references them. Delete the sync section. |
| B59 | **ELIMINATE `wireWarpMethods` INDIRECTION FOR SYNC** — `graph.syncWith()` now traverses a 3-hop chain: prototype stub (`sync.methods.js`) → `SyncController.syncWith()`. Could define sync methods as direct forwards in the WarpGraph constructor (or via getters) and delete `sync.methods.js` entirely. Same pattern applies if query/checkpoint/etc. ever get their own controllers. |
| B60 | **TEST HELPER: `mockOid(tag)`** — M9 broke 4 test files because mocks used non-hex OID strings (`'meta-oid'`, `'blob-0'`, `'corrupt-oid'`). A shared `test/helpers/mockOid.js` utility (e.g. `tag.replace(/[^0-9a-f]/gi, '0').padStart(40, '0')`) would prevent this class of breakage when validation is tightened. |
| B61 | **REMOVE DOUBLED `/** @type {any} */` ANNOTATIONS IN TESTS** — ~639 instances across service test files of `/** @type {any} */\n/** @type {any} */\nlet foo;` where the first annotation is dead noise. One-shot cleanup. |
| B62 | **DEFINE `SyncHost` INTERFACE FOR SYNCCONTROLLER** — SyncController accesses 12 private fields on the host (`_cachedState`, `_lastFrontier`, `_stateDirty`, `_persistence`, `_clock`, `_codec`, `_crypto`, `_logger`, etc.). A thin `@typedef SyncHost` in the controller file would make the coupling explicit and improve testability without mocking all of WarpGraph's internals. |
