...
| B15 | A | **TRUST PHASE B: PATCH SIGNATURE VERIFICATION** — each `PatchBuilderV2` commit includes a detached SSH signature; materialization and sync ingress verify writer identity against the trust roster's `allowed_signers` before accepting patches. Populate `requiredSignatures` and `allowedSignersPath` fields (currently reserved as `null` in trust.json v1). Mirrors shiplog's attestation mode: canonical payload `warp-patch-v1\n<tree_oid>\n<writer_id>\n<graph_name>\n` signed via `ssh-keygen -Y sign` |
| B16 | A | **TRUST PHASE B: QUORUM FOR TRUST MUTATIONS** — updating trust config requires `requiredSignatures` distinct co-signers. Two modes (following shiplog): **chain** (N sequential co-signed commits over same tree) or **attestation** (detached SSH signatures in `trust_sigs/<principal>.sig` blobs). Trust tree gains a second blob (`allowed_signers`) alongside `trust.json` for self-contained key material |
| B17 | B | **TRUST PHASE B: WRITER REVOCATION** — maintainer entries in trust.json gain a `revoked` boolean. Revoked writers' patches are rejected at materialization and sync ingress. Revocation is immediate (no grace period); re-admission requires a new trust update meeting quorum |
| B18 | B | **TRUST PHASE B: PRE-RECEIVE HOOK ENFORCEMENT** — `warp trust verify` as a server-side Git pre-receive hook on `refs/warp/**`; rejects unsigned patches or patches from writers not in the current trust roster. For GitHub/GitLab (no custom hooks), provide a CI action that runs the same verifier as a Required Status Check |
| B19 | B/C | **TRUST PHASE B: `allowlist_with_exceptions` POLICY** — implement the reserved policy: trust all listed writers by default, but support per-operation deny rules (e.g. "writer X cannot delete nodes", "writer Y can only set properties on `config:*` nodes"). Fine-grained ACL beyond shiplog's all-or-nothing model |
