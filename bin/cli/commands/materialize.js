...
async function materializeOneGraph({ persistence, graphName, writerId, ceiling }) {
  const graph = await WarpGraph.open({ persistence: /** @type {import('../../../src/domain/WarpGraph.js').FullPersistence} */ (/** @type {unknown} */ (persistence)), graphName, writerId, crypto: new WebCryptoAdapter() });
