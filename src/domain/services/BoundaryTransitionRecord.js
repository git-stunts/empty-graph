 ...
 * @private
 */
async function verifyReplayHash(btr, { crypto, codec } = /** @type {*} */ ({})) {
  try {
    ...
    return `Replay failed: ${/** @type {any} */ (err).message}`;
  }
}
 ...
 */
export async function verifyBTR(btr, key, options = /** @type {*} */ ({})) {
 ...
 */
export async function replayBTR(btr, { crypto, codec } = /** @type {*} */ ({})) {
 ...
 * @private
 */
function deserializeInitialState(U_0, { codec } = /** @type {*} */ ({})) {
 ...
 */
export function serializeBTR(btr, { codec } = /** @type {*} */ ({})) {
  const c = codec || defaultCodec;
  return c.encode({
    version: btr.version,
    h_in: btr.h_in,
 ...
 */
export function deserializeBTR(bytes, { codec } = /** @type {*} */ ({})) {
  const c = codec || defaultCodec;
