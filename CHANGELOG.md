...
- **Docs/runtime consistency cleanup** — corrected `ARCHITECTURE.md` GraphTraversal method descriptions (BFS/DFS array-returning; BFS-based `shortestPath`), removed branch-specific ROADMAP header metadata, and dropped redundant `node-gyp` apt install from `docker/Dockerfile.deno`.
